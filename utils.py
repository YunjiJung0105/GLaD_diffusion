...
import tvdatasets as custom_datasets
...
def get_dataset(dataset, data_path, batch_size=1, res=None, args=None):
    ...
    if dataset == 'CIFAR10':
        ...
        dst_train = custom_datasets.ImageNet(data_path, split="train", transform=transform) # no augmentation
        ...
        dst_test = custom_datasets.ImageNet(data_path, split="val", transform=transform)
...
def get_network(model, channel, num_classes, im_size=(32, 32), dist=False, depth=3, width=128, norm="instancenorm"):
...
def epoch(mode, dataloader, net, optimizer, criterion, args, aug):
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        if "imagenet" in args.dataset and mode != "train":
            lab = torch.tensor([class_map[j.item()] for j in lab]).to(args.device)
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args, decay="cosine", return_loss=False, test_it=100, aug=True):
    ...
    for ep in tqdm.tqdm(range(Epoch), desc='evaluate_synth'):
