...
def main(args):
    ...
    latents, cond_emb, label_syn = prepare_latents(channel=channel, num_classes=num_classes, im_size=im_size, embed_dim=args.embed_dim, G=G, class_map_inv=class_map_inv, get_images=get_images, args=args)
   
    optimizer_img = get_optimizer_img(latents=latents, cond_emb=cond_emb, G=G, args=args)
    ...
    for it in range(args.Iteration+1):
        ...
        if it in eval_it_pool and it > 0:
            save_this_it = eval_loop(latents=latents, cond_emb=cond_emb, label_syn=label_syn, G=G, best_acc=best_acc,
        ...
        if it > 0 and ((it in eval_it_pool and (save_this_it or it % 1000 == 0)) or (
            ...
            image_logging(latents=latents, cond_emb=cond_emb, label_syn=label_syn, G=G, it=it, save_this_it=save_this_it, args=args)
        ...
        for ol in range(args.outer_loop):
            ...
            with torch.no_grad():
                image_syn_w_grad = torch.cat([latent_to_im(G, syn_image_split, cond_emb_split, args) for
                                    syn_image_split, cond_emb_split, label_syn_split in
            ...
            # model_backward(latents=latents, cond_emb=cond_emb, image_syn=image_syn, G=G, args=args)
